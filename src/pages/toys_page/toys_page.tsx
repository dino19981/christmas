...
import { alphabetFilters, filterItems, getSelectedItem } from '../../data/utils';
...
export default function ToysPage() {
  ...
  const [sortSelection, setSortSelection] = useState<string>('а-я');
  ...
  function resetFilters() {
    ...
    let favorite = document.querySelector(`.row__items_input`) as HTMLInputElement;
  ...
  function selectToys(e: React.MouseEvent<HTMLDivElement, MouseEvent>, num: string, count: string) {
    ...
    for (let [key, value] of Object.entries(filters)) {
      if (key === 'shape' || key === 'size' || key === 'color') {
        value.forEach((toy: string | number) => {
          document.querySelector(`div[datatype=${toy}]`)!.classList.add('row__item_active');
        });
      }
      if (key === 'count') {
        let progressBar = document.querySelector('.noUi-connect') as HTMLDivElement;
        ...
        progressBar.style.transform = `translate(${(value[0] - 1) * 9.09}%, 0px) scale(${
          1 - 0.091 * (value[0] + 12 - value[1] - 1)
        }, 1)`;
      }
      ...
      if (key === 'onlyFavorite') {
        let input = document.querySelector('.row__items_input') as HTMLInputElement;
        if (value === true) {
          input.checked = true;
        } else {
          input.checked = false;
      ...
      <div className="container flex-container">
        <section className={styles.top_section}>
          <div className={styles.top_section__item}>
            <FiltersByValue filters={{ filters, setFilters }} setItems={setItems} />
          </div>
          <div className={styles.top_section__item}>
            <FiltersByRange filters={{ filters, setFilters }} setItems={setItems} />
          </div>
          <div className={styles.top_section__item}>
            <div className={styles.top_section__inner_third}>
              <FiltersByAlphabet setSortSelection={setSortSelection} setItems={setItems} />
              <button onClick={resetFilters} className={styles.item__button_reset}>
                Сброс фильтров
              </button>
            </div>
          </div>
        ...
        <section className="section">
          ...
          {items.length ? (
            alphabetFilters(items, sortSelection).map((toy) => {
              return <Item onClickFunc={selectToys} toy={toy} item={item} key={toy.num} />;
            })
          ) : (
            <NoToysMessage />
          )}
