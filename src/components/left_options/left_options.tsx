...
import './style.css';
import { chooseTreeData, chooseBackground, chooseGarland } from '../../tree_controll/treeControll';
...
export default function LeftOptions() {
  ...
  function snowfall() {
    ...
    let parent = document.querySelector('.section-tree__item_middle') as HTMLDivElement;
  ...
  function makeSnowfall(e: React.MouseEvent<HTMLButtonElement, MouseEvent>) {
    let target = e.target as Element;
    if (target.classList.contains('section-tree__interactive-button_active-snowfall')) {
      target.classList.remove('section-tree__interactive-button_active-snowfall');
  ...
  function savedLocalStorage() {
    ...
    let activeGarland = document.querySelector('.active-color') as HTMLDivElement;
    ...
    let star = document.querySelector('.star_show') as HTMLDivElement;
    if (activeTree) {
  ...
  function setStylesAfterLoad() {
    ...
    if (localStorageItems.tree) {
      document
        .querySelector(`.options__choose-tree[datatype='${localStorageItems.tree}']`)!
    ...
    if (localStorageItems.background) {
      document
        .querySelector(`.options__choose-background[datatype='${localStorageItems.background}']`)!
        .classList.add('options__choose-background_active');
      let background = document.querySelector('.section-tree__item_middle')! as HTMLDivElement;
      background.style.background = `url(backgrounds/${localStorageItems.background}.jpg)`;
      background.style.backgroundPosition = 'center';
      background.style.backgroundSize = 'cover';
    }
    if (localStorageItems.garland) {
      document
        .querySelector(`.options__choose-garland[datatype='${localStorageItems.garland}']`)!
        .classList.add('active-color');
      let garlendWrapper = document.querySelector('.garland__wrapper')! as HTMLDivElement;
    ...
    if (localStorageItems.star) {
      ...
      document.querySelector('.button__star')!.innerHTML = 'Убрать звезду';
    }
  ...
  return (
    <div className="section-tree__options-inner">
      ...
      <button onClick={() => localStorage.clear()} className="button button__clear-localstorage">
